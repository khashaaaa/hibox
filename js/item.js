...
Item.getConfigurationText = function(){
    ...
    for(var i in iteminfo.configurations){
        Item.ctext += configurations[i].cname+': '+configurations[i].val[$('[data-id="'+i+'"]').val()]+' <br>';
/* *** */
        var config_chy = '';
        try {
        ...
        Item.ctext_chy += $('[data-id="cname'+i+'"]').attr('config_chy')+' '+ config_chy +' <br>';
/* *** */
    }
...
Item.getConfigurationObject = function(){
    Item.confObject = {};
    for(var i in iteminfo.configurations){
        Item.confObject[i] = $("select[data-id='" + i + "']").val();
    }
}
...
Item.disableConfigurations = function () {
    $('.disabled-config').removeClass('disabled-config');

    for (var confId in Item.confObject) {
        var item_with_config = Object.assign({}, iteminfo.item_with_config);

        var confObject = Object.assign({}, Item.confObject);
        delete confObject[confId];

        var confValues = iteminfo.configurations[confId].values;
        for (var i in confValues) {
            var tmpConfValue = confValues[i].id;
            var tmpConfObj = Object.assign({}, confObject);

            tmpConfObj[confId] = tmpConfValue;

            for (var j in item_with_config) {
                var config = item_with_config[j];
                if (Item.equals(config.config, tmpConfObj)) {
                    delete item_with_config[j];

                    if (config.quantity === '0') {
                        $('[config-value-id="' + tmpConfValue + '"]').addClass('disabled-config');
                    }
                }
            }
        }
...
Item.getDefaultConfiguration = function () {
    var hash = window.location.hash.replace("#","");

    if (! hash.length) {
        for (var i in iteminfo.item_with_config) {
            if (iteminfo.item_with_config[i].quantity !== '0') {
                hash = i;
                break;
            }
        }
...
Item.setItemProperties = function (priceObj, resetMainImage) {
    ...
    if (typeof priceObj === 'undefined') {
        ...
        for (var i in priceObj.config) {
            if ($('ul[data-id="list_' + i + '"]').length && resetMainImage) {
                $('ul[data-id="list_' + i + '"] li').removeClass('list_minipic_selected');
                $('ul[data-id="list_' + i + '"] li[config-value-id="' + priceObj.config[i] + '"]').addClass('list_minipic_selected');

                var sm_pic = $('ul[data-id="list_' + i + '"] .list_minipic_selected').find('a').attr('original-image');

                if (typeof sm_pic !== 'undefined' && sm_pic !== false) {
                    var $mainImageList = $('ul.lproduct.w310li'),
                        $imageTpl = $('#tpl-main-image.pic'),
                    ...
                    $img.attr('src', $img.attr('default'));
                        $('#tpl-main-image.pic.hidden').remove()

                    $(".tpl-main-image").colorbox({rel:'group1', photo:true});

                    var newImage = new Image();
                    newImage.onload = function(){
                        $img.attr('src', sm_pic);
                        $img.closest('a').attr('href', sm_pic);
                        $img.closest('a').attr('class', 'pictures cboxElement');
                        $img.closest('a').colorbox({rel:'group1'});
                    };
                    newImage.src = sm_pic;
...
var getPriceTemplate = function (priceObject, options, promoObject) {
    ...
    for (var i in priceObject) {
        ...
        if (typeof itemPromo !== 'undefined' && itemPrice.Val !== itemPromo.Val) {
            ...
            $itemPriceEl.addClass('old-price');
            $itemPromoEl
...
Item.setDefaultConfig = function(configId){
    ...
    for(var i in iteminfo.item_with_config[configId].config){
        if(typeof i == 'undefined')
            continue;
        $('[data-id="'+i+'"]').val(iteminfo.item_with_config[configId].config[i]);
    }
}
    ...
    $.get($('#quickOrder').data('action'), params,
        function(data) {
            ...
            modalDialog(langs.reg_order, layout,
                ...
                { confirm: trans.get('make_order'), cancel: trans.get('continue_shopping') },
                function (body) {
                    hideOverlay();
                }, undefined, 3
            );
...
function init(iteminfoAjax, langsAjax){
    ...
    $('.switch').click(function() {
        ...
        if (isImage) {
            ...
            newImage.onload = function(){
                ...
                $img.closest('a').attr('class', 'pictures');
                $img.closest('a').colorbox({rel:'group1', photo:true});
    ...
    var config = Item.getDefaultConfiguration();
    changeconfig(config);
}
