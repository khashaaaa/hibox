...
class UploadHandler
{
    ...
    function __construct($options = null, $initialize = true, $error_messages = null, $uploadFolder = false) {
        $this->options = array(
            'script_url' => $this->get_full_url().'/',
            'upload_dir' => CFG_APP_ROOT . ($uploadFolder ? $uploadFolder : '/uploaded/'),
            'upload_url' => $this->get_full_url() . ($uploadFolder ? $uploadFolder : '/uploaded/'),
            'user_dirs' => false,
            'mkdir_mode' => 0755,
            'param_name' => 'files',
    ...
    public function get_full_url() {
        $https = General::getConfigValue('use_https');
        if (!$https) {
    ...
    protected function get_upload_path($file_name = null, $version = null, $includeFileNameInPath = true, $options = null) {
        ...
        if(!$includeFileNameInPath) $file_name = '';
    ...
    protected function create_scaled_image($file_name, $version, $options) {
        ...
        if (!empty($version) && $version !== 'original') {
            $version_dir = $this->get_upload_path($file_name, $version, false, $options);
            if (!is_dir($version_dir)) {
                mkdir($version_dir, $this->options['mkdir_mode'], true);
            }
            $new_file_path = $version_dir.'/'.(! empty($options['name']) ? $options['name'] . '.' . (strtolower(substr(strrchr($file_name, '.'), 1))) : $file_name);
        } else {
        ...
        $max_width = isset($options['max_width']) ? $options['max_width'] : 1024;
        $max_height = isset($options['max_height']) ? $options['max_height'] : 780;
        $scale = min(
            $max_width / $img_width,
        ...
        switch (strtolower(substr(strrchr($file_name, '.'), 1))) {
            ...
            case 'gif':
                ...
                break;
            case 'png':
                $memory_limit = $this->get_config_bytes(ini_get('memory_limit'));
                $filePixels = $img_width * $img_height * 8;
                if ($filePixels >= $memory_limit ) {
                    $src_img = null;
                    return array (
                        'result' => false,
                        'error' => "To big picture. Server needs more memory",
                    );
                } else {
                    @imagecolortransparent($new_img, @imagecolorallocate($new_img, 0, 0, 0));
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error,
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            $upload_dir = $this->get_upload_path($file->name, null, false);
            if (!is_dir($upload_dir)) {
                mkdir($upload_dir, $this->options['mkdir_mode'], true);
            }
            $file_path = $this->get_upload_path($file->name);
            $file->uploaded_url = $file_path;
            ...
            if ($file_size === $file->size) {
                ...
                $file->url = $this->get_download_url($file->name);
                foreach($this->options['image_versions'] as $version => $options) {
                    $result = $this->create_scaled_image($file->name, $version, $options);
                    if ($result['result']) {
                        if (!empty($version)) {
                            $file->{$version.'_url'} = $this->get_download_url(
                                $file->name,
