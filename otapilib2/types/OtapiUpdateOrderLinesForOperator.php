...
class OtapiUpdateOrderLinesForOperator extends BaseOtapiType{
    /**
     * @return string
     */
    ...
    public function GetOrderLineIds(){
        $value = isset($this->xmlData->orderLineIds) ? (string)$this->xmlData->orderLineIds : false;
        $propertyType = 'string';
        return $propertyType == 'boolean' ? $value == 'true' : $value;
    }
    /**
     * @return string
     */
    public function GetXmlUpdateData(){
        $value = isset($this->xmlData->xmlUpdateData) ? (string)$this->xmlData->xmlUpdateData : false;
        $propertyType = 'string';
        return $propertyType == 'boolean' ? $value == 'true' : $value;
    }
}