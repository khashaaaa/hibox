...
<div id="popular" class="block-container" >
    ...
    <ul id="sortable3" class="sortable">
        <? foreach ($set3 as $item): ?>
            <li class="sortlist" id="se3<?=$item['id']?>">
    ...
    <? } ?>
    <br/>
    <div id="save3" style="display:none;float:right;">
    ...
    <form id="form3" action="<?=BASE_DIR;?>index.php?sid=<?=$GLOBALS['ssid'];?>&amp;do=add&amp;cmd=randomset" method="post">
    ...
    <br/><br/>
    <form id="form3del" action="<?=BASE_DIR;?>index.php?sid=<?=$GLOBALS['ssid'];?>&amp;do=delall&amp;cmd=randomset" method="post">
        <input type="hidden" name="catid" value="<?=$catid?>"/>
