...
function updateOrderSummary(ev, updatePrice, items) {
    var totalPrice = 0;
    calculateTotalWeightByRows();
    var totalWeight = $('span.total-weight').data('weight') ? $('span.total-weight').data('weight').toString().replace(' ', '') : 0;
    var priceSign = '';
    ...
    var getTotalCost = function(items) {
        $.ajax({
            async : true,
            type: 'POST',
            dataType: 'json',
...
$(document).ready(function(){
	...
	$('.notepad input.weight-value').change(function(e){
