...
class HSTemplateDisplay
{
	 ...
	 */
	  ...
	  * @return  void
	  */
	 function addTemplate($aTemplateName, $aTemplateFile, $aTemplatePath = null, $use_tpl_stack = true)
	 {
	     if (!$aTemplatePath) {
	 ...
	 {
        if (!is_writable($this->_HSTemplate->_options['cache_path'] )) 
        {
            trigger_error('HSTemplate Error: Cannot write directory "' . $this->_HSTemplate->_options['cache_path'] . '"', E_USER_ERROR);
	    }
	    $this->_caching          = true;
	  ...
	  * @return  void
	  */
	 function unsetCache($aId)
	 {
	    $this->_caching = false;
        $this->_caching_id = null;
	 	return true;
	 }
	 ...
	 function isCached()
	 {
        ...
        //print_r('<br>clear_cache_'.$this->_name.' ');
	    if (isset($_SESSION['clear_cache_'.$this->_name]))
        {
            //print('<= clear');
            Session::clear('clear_cache_'.$this->_name);
     ...
     * @param bool $allowMultipleInclude
     * @return  rettype  return
     */
	 function fetch($aTemplate = null, $aDisplay = false, $allowMultipleInclude = false)
	 {
	    ...
	    if ($this->_caching && $this->isCached()) {
	        ...
	        ob_start();
    	 	$this->_fetch($aTemplate, $allowMultipleInclude);
	        $cache = ob_get_contents();
	        ob_end_clean();
     ...
     * @return  rettype  return
     */
	 function _fetch($aTemplate = null, $allowMultipleInclude = false)
	 {
	 	if ($aTemplate) {
	 	   ...
	 	   $this->_display($aTemplate, $allowMultipleInclude);
	 	} else {
    ...
    * @access  private
    * @param   string     $aTemplate  template name
    * @param $allowMultipleInclude
    * @return  rettype  return
    */
    function _display($aTemplate, $allowMultipleInclude = true)
    {
        $vars = array();
        if (sizeof($this->_HSTemplate->_vars) > 0) {
            extract($this->_HSTemplate->_vars);
            $vars = array_merge($vars, $this->_HSTemplate->_vars);
        }
        if (isset($this->_vars[HSTEMPLATE_DISPLAY_MAIN]) && sizeof($this->_vars[HSTEMPLATE_DISPLAY_MAIN]) > 0) {
            extract($this->_vars[HSTEMPLATE_DISPLAY_MAIN]);
            $vars = array_merge($vars, $this->_vars[HSTEMPLATE_DISPLAY_MAIN]);
        }
        if (sizeof($this->_vars[$aTemplate]) > 0) {
            extract($this->_vars[$aTemplate]);
            $vars = array_merge($vars, $this->_vars[$aTemplate]);
        }

        $template = str_replace(CFG_APP_ROOT, '', $this->_templates[$aTemplate]);
        $before = Plugins::runEvent('onBeforeHSTemplateDisplay_' . str_replace(array('\\', '/', '-', '.'), '_', $template), $vars);
        $after = Plugins::runEvent('onAfterHSTemplateDisplay_' . str_replace(array('\\', '/', '-', '.'), '_', $template), $vars);
        unset($vars);

        print $before;
        if ($allowMultipleInclude) {
            include($this->_templates[$aTemplate]);
        } else {
    ...
    protected function generatePageTitle($items)
    {
        $title = array();
