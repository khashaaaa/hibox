...
class Minify_Cache_File {
    ...
    public function __construct($path = '', $fileLocking = false)
    {
        ...
        if (! is_dir($path)) // фикс ошибки при первом обращении к минимизатору, при отсутствии папки кэша
        ...
        if ($this->_locking) {
            ...
            if (!$fp) {
                if (! is_dir($this->_path)) {
                    if (! mkdir($this->_path, 0755, true)) { // создать директорию рекурсивно
                        return false;
                    } else {
                        $fp = fopen($this->_path . '/' . $id, "w"); // создать файл
                        fclose($fp);// закрыли созданный файл
                        $fp = fopen($this->_path . '/' . $id, 'rb'); // открыли его для дальнейшей работы библиотеки
                        if(!$fp)
                            return false;
                    }
                }
