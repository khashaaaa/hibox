...
// костыль для js - скриптов, подключаемых внутри других js - скриптов (например tiny_mce с плагинами)
$request_uri_arr = explode('?', $_SERVER['REQUEST_URI']);
$parameters_arr = explode('&', $request_uri_arr[1]);
if (count($parameters_arr) > 1) {
    $_GET['f'] = str_replace('f=', '', $parameters_arr[0]);
}
