...
class UrlRule
{
     ...
     * @param RequestWrapper $request the request component
     * @return array|bool the parsing result. The route and the parameters are returned as an array.
     * If `false`, it means this rule cannot be used to parse this path info.
     */
    public function parseRequest($request, $pathInfo = null)
    {
        ...
        $pathInfo = !is_null($pathInfo) ? $pathInfo : $request->path();
        $pathInfo = trim($pathInfo, '/');

        if ($this->host !== null) {
            $pathInfo = strtolower($request->env('SERVER_NAME')) . ($pathInfo === '' ? '' : '/' . $pathInfo);
        }
    ...
    public function createUrl($route, $params)
    {
        ...
        if (General::IsFeatureEnabled('Seo2')) {
            ...
            $url = '?' . Router::ROUTE_PARAM . '=' . implode('/', $routeParts);
            foreach ($tr as $routeParamName => $routeParamValue) {
                $url .= '/' . $routeParamValue;
            }
