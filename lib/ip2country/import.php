...
if(!$ip2c->mysql_con())
die('Could not connect to database ' . mysqli_error($ip2c->get_mysql_con()));

if(!($r=mysqli_query($ip2c->get_mysql_con(), "SHOW VARIABLES LIKE 'max_allowed_packet'")))die( 'mysql_error:' . mysql_error($ip2c->get_mysql_con()) . '<br />');
$row=mysqli_fetch_assoc($r);

$fsize=@filesize('GeoIPCountryWhoisChina.csv');
if(!$fsize)
...
$f=@fopen('GeoIPCountryWhoisChina.csv','r');
...
if(!mysqli_query($ip2c->get_mysql_con(), "INSERT into " .$ip2c->table_name . "(`begin_ip`,`end_ip`,`begin_ip_num`,`end_ip_num`,`country_code`,`country_name`) values(" . $str . ")"))die ('mysql_error: ' . mysqli_error($ip2c->get_mysql_con()) . '<br />');
